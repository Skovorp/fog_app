...
enum SessionPDF {
    ...
    private static func drawGraph(scores: [Float], in rect: CGRect, showFogThreshold: Bool) {
        ...
        // Y-axis gridlines + labels (0, 0.25, 0.5, 0.75, 1.0). Every head
        // normalizes its output into [0, 1] upstream, so this scale is
        // shared across FoG / chair / walking / tapping.
        ...
        // Score line. Scores are already in [0, 1] upstream — clamp is
        // defensive against any pre-normalization Sessions decoded from
        // disk (unlikely now, but cheap insurance).
        ctx.setStrokeColor(UIColor.lucheInk.cgColor)
